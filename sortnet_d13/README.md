...
**Paper:** `paper/main.pdf` is the manuscript as an academic paper (LaTeX source `paper/main.tex`, built with `make -C paper`): abstract, introduction, preliminaries, methods, calibration on 28 channels, the 27/152/13 network as Theorem 1 with its verification transcript, the size-optimality statement for Wang's prefix, the 30- and 32-channel negative results as Theorems 2 and 3 with the run tables, discussion, limitations, reproducibility, confidence, references, and appendices with per-instance CNF sizes and times. `manuscript.md` is the same content in plain Markdown.
